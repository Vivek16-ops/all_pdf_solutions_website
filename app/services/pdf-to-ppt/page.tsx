...
const PDFtoPPTPage = () => {
  ...
  const [fileInfo, setFileInfo] = useState<{ size: string, pages?: number, estimatedTime?: string } | null>(null);  const [showTooltip, setShowTooltip] = useState<string | null>(null);
  const [userRating, setUserRating] = useState(0);
  const [lastTooltipTime, setLastTooltipTime] = useState<number>(0);

  // Show helpful tooltips with better control
  useEffect(() => {
    const tips = [
    ...
    const showRandomTip = () => {
      ...
      const timeSinceLastTip = currentTime - lastTooltipTime;

      // Only show tooltip if: no tooltip currently showing, not converting, not signed in or no file, and 20 seconds have passed
      if (!showTooltip && !isConverting && (!isSignedIn || !file) && timeSinceLastTip > 20000) {
        const randomTip = tips[Math.floor(Math.random() * tips.length)];
        setShowTooltip(randomTip);
        setLastTooltipTime(currentTime);
        setTimeout(() => setShowTooltip(null), 6000);
      }
    };

    const interval = setInterval(showRandomTip, 25000); // Check every 25 seconds
    return () => clearInterval(interval);
  }, [showTooltip, isConverting, isSignedIn, file, lastTooltipTime]);
          ...
          <div className="max-w-4xl mx-auto px-4 py-12 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              ...
              >
                ...
                PDF to PowerPoint Converter                <motion.span
  ...
  return (
    <>
      <Navbar />
      <motion.main
        className={`min-h-screen pt-10 transition-colors duration-500 ${isDark
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">          <motion.div
        ...
        >
          ...
          transition={{ duration: 10, repeat: Infinity, ease: "easeInOut", delay: 2, type: "tween" }}
        >
            ✨
          </motion.div>
          <motion.div
            className={`absolute bottom-40 left-20 text-4xl ${isDark ? 'text-blue-800/20' : 'text-blue-200/30'}`} animate={{
              scale: [1, 1.2, 1],
              rotate: [0, 360]
            }}
        ...
        >          {/* Header Section */}
          <div className="text-center mb-8 md:mb-12 px-2">
            ...
            >
              ✨ Transform your PDF files into dynamic PowerPoint presentations with magical ease!              <motion.span
                animate={{ scale: [1, 1.2, 1] }}
            ...
            {!isSignedIn && (
              <motion.div
                className={`mt-4 md:mt-6 p-3 md:p-4 rounded-xl backdrop-blur-sm mx-2 ${isDark ? 'bg-purple-900/30 border border-purple-800/50' : 'bg-purple-100/50 border border-purple-200'
                  }`}
          ...
          <motion.div
            className={`rounded-2xl md:rounded-3xl p-4 sm:p-6 md:p-8 backdrop-blur-sm transition-all duration-300 relative overflow-hidden mx-2 sm:mx-0 ${isDark
              ...
              >
                ...
                >                  {!isSignedIn ? (
                  ...
                  >
                    ...
                    >
                      ...
                      {fileInfo && (
                        ...
                        >
                          ...
                          <motion.button
                            ...
                            className={`w-full py-2 px-4 rounded-md transition-all duration-200 flex items-center justify-center gap-2 ${isDark
                      ...
                      <h3 className="text-lg sm:text-xl md:text-2xl font-bold mb-2 md:mb-3 flex items-center justify-center gap-2 md:gap-3 flex-wrap">
                        ...
                        <span className="text-center">Selected File</span>                        <motion.span
                      ...
                      <motion.p
                        className="text-xs sm:text-sm opacity-75" animate={{ opacity: [0.5, 1, 0.5] }}
                        transition={{ duration: 2, repeat: Infinity, type: "tween" }}
                      >
                  ...
                  >
                    ...
                    <h3 className="text-lg sm:text-xl md:text-2xl font-bold mb-2 md:mb-3">                        <motion.span
                      animate={{ scale: [1, 1.2, 1] }}
                    ...
                    <motion.p
                      className="text-sm sm:text-base md:text-lg" animate={{ opacity: [0.7, 1, 0.7] }}
            ...
            <AnimatePresence>
              ...
              >
                ...
                >                  <motion.div
                  ...
                  <motion.h3
                    className={`text-xl md:text-2xl font-bold mb-3 ${isDark ? 'text-green-300' : 'text-green-700'}`}
                  ...
                  {/* PPT Preview Section - Hidden on Mobile */}
                  <div className="hidden md:block mb-6">
                    <motion.div
                      className={`max-w-5xl mx-auto relative rounded-xl border-2 overflow-hidden shadow-2xl ${isDark
                    ...
                    >
                      {/* Preview Header */}
                      <div className={`px-4 py-3 border-b flex items-center justify-between ${isDark
                          ? 'bg-gray-800 border-gray-700'
                          : 'bg-gray-50 border-gray-200'
                        }`}>
                        <div className="flex items-center gap-3">
                          <motion.div
                            className={`p-2 rounded-lg ${isDark ? 'bg-purple-900/30' : 'bg-purple-100'
                              }`}
                          ...
                          <div className="text-left">
                            <h3 className={`font-medium text-sm ${isDark ? 'text-purple-200' : 'text-purple-800'
                              }`}>
                              PowerPoint Presentation Preview
                            </h3>
                            <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'
                              }`}>
                              {previewData?.totalSlides || 1} slides • Ready for download
                            </p>
                          </div>
                        </div>
                        <motion.div
                          ...
                          {previewData?.slides && previewData.slides.length > 0 ? (
                            <>
                              ...
                              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 max-h-96 overflow-y-auto">
                                {previewData.slides.slice(0, 4).map((slide, index) => (
                                  <motion.div
                                    key={index}
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center gap-2 mb-3">
                                      ...
                                      <h4 className={`font-medium text-sm ${isDark ? 'text-purple-300' : 'text-purple-700'
                                        }`}>
                                        Slide {index + 1}
                                      </h4>
                                    </div>
                                    ...
                                    {/* Slide Title */}
                                    <h5 className={`font-bold text-sm mb-2 ${isDark ? 'text-white' : 'text-gray-900'
                                      }`}>
                                      {slide.title}
                                    </h5>
                                    ...
                                    <div className="space-y-1">
                                      {slide.contentPreview?.slice(0, 3).map((content, contentIndex) => (
                                        <div
                                          key={contentIndex}
                                      ...
                                      {slide.contentPreview && slide.contentPreview.length > 3 && (
                                        <div className={`text-xs italic ${isDark ? 'text-gray-500' : 'text-gray-500'
                                          }`}>
                                          ... and {slide.contentPreview.length - 3} more points
                                        </div>
                                      )}
                                    </div>
                                  </motion.div>
                                ))}
                              </div>                                {/* Show more slides indicator */}
                              {previewData.slides.length > 4 && (
                                <motion.div
                                  className={`text-center p-4 rounded-lg border-2 border-dashed ${isDark
                                ...
                                >
                                  <p className="text-sm font-medium flex items-center justify-center gap-2">
                                    <span>📊</span>
                                    +{previewData.slides.length - 4} more slides in your presentation
                                    <span>✨</span>
                                  </p>
                                </motion.div>
                              )}
                            ...
                            <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'
                              ...
                              Preview will be available after conversion
                            </p>
                          </div>
                          )}
                        </div>
                  ...
                  <motion.button
                    onClick={downloadPpt}
                    className={`px-8 py-4 rounded-xl font-bold text-lg transition-all relative overflow-hidden ${isDark
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 relative z-10">                        <motion.span
                      animate={{
                        y: [0, -3, 0],
                        scale: [1, 1.2, 1]
                      }}
                    ...
                    >
                      ...
                      <span>Download PowerPoint</span>                        <motion.span
                        animate={{
                          rotate: [0, 360],
                          scale: [1, 1.1, 1]
                        }}
        ...
        <AnimatePresence>
          {showTooltip && (
            <motion.div
              className={`fixed top-4 right-4 p-4 rounded-xl backdrop-blur-sm border z-40 max-w-sm ${isDark
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  onClick={() => {
                    setShowTooltip(null);
                    setLastTooltipTime(Date.now()); // Prevent immediate re-showing
                  }}
                  className={`ml-auto text-lg hover:scale-110 transition-transform ${isDark ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-800'
                    }`}
        ...
        <AnimatePresence>
          {pptFile && userRating === 0 && (
            <motion.div
              className={`fixed bottom-4 left-4 p-4 rounded-xl backdrop-blur-sm border z-40 ${isDark
